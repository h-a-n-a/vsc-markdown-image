    ...
    async () => {
      ...
      try {
        ...
        for (let i = 0; i < urls.length; i++) {
          let selection = await utils.getAlt(
            config.base.altFormat,
            images[i],
            context
          );
          if (
          ...
          if (config.base.uploadMethod !== 'Data URL') {
            if (config.base.urlEncode) {
              ...
              urls[i] = urls[i].replace(/ /g, '%20');
            }
            let text = await utils.formatCode(
              urls[i],
              selection,
              maxWidth[i],
              config.base.codeType,
              config.base.codeFormat || '![${alt}](${src})'
            );
